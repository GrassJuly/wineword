 ...
 */
@Deprecated
public class RJListView extends ListView implements OnScrollListener {
    // listener
    private OnScrollListener mScrollListener;
    private RJRefreshListener mListViewListener;

    // widget
    private RJListViewHeader mHeaderView; // 头部控件
    private TextView mHeaderTimeView; // 头部显示上次刷新时间的控件
    private RelativeLayout mHeaderViewContent;// 头部控件的layout（依靠它计算头部的显示高度）
    private RJListViewFooter mFooterView; // 底部控件
    ...
    public RJListView(Context context) {
        super(context);
        initWithContext(context);
    }

    public RJListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initWithContext(context);
    }

    public RJListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initWithContext(Context context) {
        ...
        // 初始化头部
        mHeaderView = new RJListViewHeader(context);
        ...
        // 初始化底部
        mFooterView = new RJListViewFooter(context);
        ...
        default:
            ...
            if (getFirstVisiblePosition() == 0) {
                ...
                if (mEnablePullRefresh
                    ...
                    mHeaderView.setState(RJListViewHeader.RefreshState.STATE_REFRESHING);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
            if (mHeaderView.getVisibleHeight() > mHeaderViewHeight) {
                mHeaderView.setState(RJListViewHeader.RefreshState.STATE_READY);
            } else {
                mHeaderView.setState(RJListViewHeader.RefreshState.STATE_NORMAL);
            }
        }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading) {
            if (height > PULL_LOAD_MORE_DELTA) {
                // 上拉足够高时，加载更多
                mFooterView.setState(RJListViewFooter.LoadMoreState.STATE_READY);
            } else {
                mFooterView.setState(RJListViewFooter.LoadMoreState.STATE_NORMAL);
            }
        }
    ...
    private void startLoadMore() {
        mPullLoading = true;
        mFooterView.setState(RJListViewFooter.LoadMoreState.STATE_LOADING);
        if (mListViewListener != null) {
     ...
     * 设置自己的监听器
     */
    public void setOnRefreshListener(RJRefreshListener l) {
        mListViewListener = l;
    }
